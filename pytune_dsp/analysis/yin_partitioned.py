...
def build_octave_bands_8(fmin: float = A0_HZ, fmax: float = C8_HZ) -> List[Band]:
...
def build_octave_bands_4x2(fmin: float = A0_HZ, fmax: float = C8_HZ) -> List[Band]:
...
def build_note_centered_bands_cents(
    ...
    → Ex: pour A0 (27.5 Hz, window=300c) :
       band ≈ [23.1, 32.7] Hz, comme dans les logs [EXP-YIN].
    """
    bands: List[Band] = []
